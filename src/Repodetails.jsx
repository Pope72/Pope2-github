...
function RepoDetails() {
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/Pope72/${id}`)
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/Pope72/${id}/branches`)
    .then((response) => (response.json()))
    .then((data) => {
      setBranch(data)
    })
  ...
  useEffect(() => {
    fetch(`https://api.github.com/repos/Pope72/${id}/deployments`)
    .then((response) => (response.json()))
    .then((data) => {
      setDeployment(data)
    })
  ...
  return (
    <div id="repodetail">
        <div className="repodetail-card">
            ...
            <p>Live site: {deployment.length === 0 ? `none` : <a href={`https://Pope72.github.io/${details.name}`}>mbonamensa.github.io/{details.name}</a>}</p>
